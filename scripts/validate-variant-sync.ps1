...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    ...
    $OutputDirectory = Join-Path $tempRoot "q-workflow-variant-sync"
}
...
Invoke-Step "PowerShell parse" {
    $scripts = @(
        ...
        (Join-Path $public "scripts\sync-workflow-bootstrap.ps1"),
        ...
        (Join-Path $company "scripts\sync-workflow-bootstrap.ps1"),
