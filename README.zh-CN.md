...
> **当前公开包：** `main` 分支的 [v1.2 范围说明](docs/releases/q-workflow-v1.2.md)。
> 安装面向 Windows，需要 PowerShell、Git 和 Python 3.10+。
> 项目仍处于早期阶段，不是托管云服务，也不是大模型。
...
- 当前公开包不包含已撤回的像素画、Canvas 游戏和 HTML 界面技能。
  历史标签不等于当前受支持的包，具体见 [v1.2 范围说明](docs/releases/q-workflow-v1.2.md)。
- 验证针对具体的工作流与安装行为，不代表模型智力、创作质量或在线服务可用性得到证明。
- 技能是给智能体的行为指导，不是安全沙箱，也不能代替智能体本身的权限控制和你的检查。
...
更新工具不会悄悄删除已有安装中被撤回的技能。
将这些明确命名的目录移出技能发现范围前，先保留自定义工作。
...
| 核对公开包范围及验证方法 | [v1.2 范围与验证](docs/releases/q-workflow-v1.2.md)（英文） |
