...
check: run the owning domain's pointer validator when available; otherwise inspect one authority record plus hashes/diffs of its governing files. Recovery scenarios must prove that an old report/image/binary cannot override the pointer and that a material user correction locks execution until represented. Font work uses q-font-design `scripts/font_authority_gate.py`.
