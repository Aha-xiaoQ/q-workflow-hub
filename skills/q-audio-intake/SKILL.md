...
2. **Prepare audio deterministically.**
   Use `--engine none` to download or extract audio without calling an STT
   provider. This is a smoke test only, not a quality benchmark.
...
4. **Benchmark quality against reference subtitles.**
   Prefer real videos with downloadable official/platform subtitles. Compare
   the STT transcript to the reference with `score`; do not rely on subjective
...
useful fast-draft path; benchmark against subtitles before treating output as
final. Use `workflows/install-whisper-cpp.md` to make this setup reproducible
outside `%TEMP%`.
...
Synthetic samples are useful for smoke tests only. A provider is not ready until
it passes at least one real closed-loop benchmark: official/platform subtitle
