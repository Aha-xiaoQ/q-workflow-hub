...
| C — documented gap, higher model available | Four conflicting current primary sources plus local constraints determine an irreversible design decision; `gpt-5.6-sol` is available. | One bounded `gpt-5.6-sol` child; reasoning is independently chosen and justified. | actual model, effort reason, `cost_posture`, `action_authority: unchanged`, source/evidence output cap, stop condition. |
